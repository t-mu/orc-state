...
describe('cli/remove-worker.mjs', () => {
  it('removes worker without session handle', async () => {
    ...
    process.argv = ['node', 'cli/remove-worker.mjs', 'bob'];
    setStateDirEnv(dir);
    try {
      await import('./remove-worker.mjs');
    } finally {
  ...
  it('does not stop session when --keep-session is set', async () => {
    ...
    const oldArgv = process.argv;
    const oldStateDir = process.env.ORCH_STATE_DIR;
    process.argv = ['node', 'cli/remove-worker.mjs', 'bob', '--keep-session'];
    setStateDirEnv(dir);
    try {
      await import('./remove-worker.mjs');
    } finally {
