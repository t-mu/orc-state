...
/**
 * cli/status.mjs
 * Usage: node cli/status.mjs [--json] [--mine --agent-id=<id>]
 *
 * Print current orchestrator state from base files + events.jsonl.
 */
import { flag } from '../lib/args.mjs';
import { buildAgentStatus, buildStatus, formatAgentStatus, formatStatus } from '../lib/statusView.mjs';
...
if (mine) {
  const agentStatus = buildAgentStatus(STATE_DIR, agentId);
  ...
  if (json) {
    ...
    console.log(formatAgentStatus(agentStatus, agentId));
  }
  process.exit(0);
}
