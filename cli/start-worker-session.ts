...
console.log('orc start-worker-session (debug worker session control)');

workerId = await promptAgentId(workerId);
if (!workerId) {
  console.error('Usage: orc start-worker-session <worker_id> --provider=<codex|claude|gemini> [--role=<worker|reviewer|scout>] [--force-rebind]');
  process.exit(1);
}
if (workerId === 'master') {
