...
const COORDINATOR_SCRIPT_PATH = resolve(import.meta.dirname, '..', 'coordinator.mjs');
...
function removeCoordinatorPidFileIfMatches(pid) {
...
function isCoordinatorCommandPid(pid) {
...
function coordinatorStatus() {
  ...
  try {
    ...
    if (e.code === 'ESRCH') return { running: false, pid };
    return { running: isCoordinatorCommandPid(pid), pid };
  }
}
...
async function stopCoordinator(pid) {
...
try {
  ...
  console.error(`Failed preparing master session: ${error?.message ?? 'unknown error'}`);
  process.exit(1);
}
...
const cliResult = await new Promise((resolve) => {
  ...
  masterPty.onExit(({ exitCode, signal }) => resolve({ type: 'close', code: exitCode, signal }));
...
if (cliResult.type === 'error') {
  console.error(
    `Failed to start master provider CLI '${binary}' for ${master.provider}: ${cliResult.error?.message ?? 'unknown error'}`,
  );
  markMasterOffline();
  process.exit(1);
