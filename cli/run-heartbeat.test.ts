...
beforeEach(() => {
  dir = createTempStateDir('orc-run-heartbeat-test-');
  seedState();
});
...
describe('cli/run-heartbeat.ts', () => {
  it('emits heartbeat event for an in_progress run', () => {
    const result = runCli(['--run-id=run-1', '--agent-id=worker-1']);
    expect(result.status).toBe(0);
    expect(result.stdout).toContain('heartbeat: run-1 (worker-1)');
    const events = readEvents();
    const event = events.find((e) => e.event === 'heartbeat' && e.run_id === 'run-1');
  ...
  it('exits 1 when --run-id is missing', () => {
    ...
    expect(result.stderr).toContain('Usage: orc run-heartbeat');
  });
  ...
  it('exits 1 when claim is not in_progress', () => {
  ...
  it('exits 1 when lease has expired', () => {
    writeFileSync(join(dir, 'claims.json'), JSON.stringify({
      version: '1',
      claims: [{
        run_id: 'run-1',
...
function runCli(args: string[]) {
  return spawnSync('node', ['cli/run-heartbeat.ts', ...args], {
    cwd: repoRoot,
