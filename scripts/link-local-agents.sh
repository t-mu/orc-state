...
usage() {
  cat <<'EOF'
Usage: scripts/link-local-agents.sh [codex|claude|all]

Create local development symlinks so provider agent discovery points at the
repo's canonical agents/ source directory.

Defaults to: all

Behavior:
- creates .codex/agents -> ../agents
...
if [[ ! -d "$SOURCE_DIR" ]]; then
  echo "agents/ directory not found at: $SOURCE_DIR" >&2
  exit 1
fi
...
link_provider() {
  ...
  local link_path="$target_dir/agents"
  local relative_source="../agents"
