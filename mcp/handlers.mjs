...
import { describeAutoTargetFailure, selectAutoTarget } from '../lib/dispatchPlanner.mjs';
import { appendSequencedEvent, readRecentEvents } from '../lib/eventLog.mjs';
...
import { findTask, getNextTaskSeq, readClaims, readJson } from '../lib/stateReader.mjs';
import { canAgentExecuteTask, evaluateTaskEligibility, formatRoutingReasons } from '../lib/taskRouting.mjs';
...
function defaultActorId(stateDir) {
  const master = listAgents(stateDir).find((agent) => agent.role === 'master');
...
export function handleListTasks(stateDir, { status, epic } = {}) {
  ...
  let tasks = (backlog.epics ?? []).flatMap((epicObj) =>
    (epicObj.tasks ?? []).map((task) => ({ ...task, epic_ref: epicObj.ref })),
  );
...
export function handleListAgents(stateDir, { role, include_dead = false } = {}) {
  if (role != null && !AGENT_ROLES.has(role)) {
  ...
  const activeClaimsByAgent = new Map();
...
export function handleListStalledRuns(stateDir, { stale_after_ms = 600_000, now_ms } = {}) {
  if (!Number.isInteger(stale_after_ms) || stale_after_ms < 0) {
  ...
  return (readClaims(stateDir).claims ?? [])
    ...
    .filter((claim) => {
      ...
      return (now - new Date(timestamp).getTime()) > stale_after_ms;
    })
    .map((claim) => ({
      ...claim,
...
export function handleGetStatus(stateDir, { include_done_count = false } = {}) {
  ...
  if (include_done_count) {
    status.task_counts.done = 0;
    status.task_counts.released = 0;
  }

  for (const epic of backlog.epics ?? []) {
    for (const task of epic.tasks ?? []) {
      if (Object.hasOwn(status.task_counts, task.status)) {
        status.task_counts[task.status] += 1;
      }
...
export function handleGetAgentWorkview(stateDir, { agent_id } = {}) {
  ...
  for (const epic of backlog.epics ?? []) {
    for (const task of epic.tasks ?? []) {
      ...
      const unmetDependencies = (task.depends_on ?? []).filter((dependency) => !doneSet.has(dependency));
...
export function handleCreateTask(stateDir, args = {}) {
  ...
  if (!TASK_TYPES.has(task_type)) throw new Error(`Invalid task_type: ${task_type}`);
  if (!TASK_PRIORITIES.has(priority)) throw new Error(`Invalid priority: ${priority}`);
  if (!ACTOR_ID_RE.test(actor_id)) throw new Error(`Invalid actor-id: ${actor_id}. Must match ^[a-z0-9][a-z0-9-]*$.`);
  if (owner && !ACTOR_ID_RE.test(owner)) throw new Error(`Invalid owner: ${owner}. Must match ^[a-z0-9][a-z0-9-]*$.`);
  ...
  const taskSlug = ref ?? slugify(title);
  ...
  return withLock(join(stateDir, '.lock'), () => {
    if (actor_id !== 'human') {
      ...
      if (!actorExists) {
        throw new Error(`Actor agent not found: ${actor_id}. Registered agents: ${allAgents.map((agent) => agent.agent_id).join(', ') || '(none)'}`);
      }
    }
...
export function handleUpdateTask(stateDir, args = {}) {
  ...
  if (!task_ref) throw new Error('task_ref is required');
  if (!ACTOR_ID_RE.test(actor_id)) throw new Error(`Invalid actor_id: ${actor_id}. Must match ^[a-z0-9][a-z0-9-]*$.`);
  assertStringArray(acceptance_criteria, 'acceptance_criteria');
  assertStringArray(depends_on, 'depends_on');
...
export function handleDelegateTask(stateDir, args = {}) {
  ...
  if (!task_ref) throw new Error('task_ref is required');
  if (!ACTOR_ID_RE.test(actor_id)) throw new Error(`Invalid actor-id: ${actor_id}. Must match ^[a-z0-9][a-z0-9-]*$.`);
  if (!TASK_TYPES.has(task_type)) throw new Error(`Invalid task type: ${task_type}`);
  ...
  return withLock(join(stateDir, '.lock'), () => {
    ...
    let epicRef = null;
    for (const epic of backlog.epics ?? []) {
      task = (epic.tasks ?? []).find((candidate) => candidate.ref === task_ref);
      if (task) {
        epicRef = epic.ref;
        break;
      }
    ...
    if (assignedTarget) {
      ...
      const evaluation = evaluateTaskEligibility({ ...task, task_type }, target);
...
export function handleCancelTask(stateDir, args = {}) {
  const {
    task_ref,
    reason = null,
    actor_id = defaultActorId(stateDir),
  ...
  const result = withLock(join(stateDir, '.lock'), () => {
    ...
    task.status = 'blocked';
    task.updated_at = now;
    task.blocked_reason = reason ?? 'cancelled';
    atomicWriteJson(backlogPath, backlog);
...
export function handleRespondInput(stateDir, { run_id, agent_id, response, actor_id } = {}) {
  ...
  const effectiveActorId = actor_id ?? defaultActorId(stateDir);
  ...
  try {
    setRunInputState(stateDir, run_id, agent_id, { inputState: null });
  } catch {
