...
describe('mcp stdio protocol', () => {
  ...
  it('executes read tools over JSON-RPC', async () => {
    ...
    expect(payload.some((task) => task.ref === 'project/todo-one')).toBe(true);
  });

  it('lists and executes get_agent_workview over JSON-RPC', async () => {
    const result = await callTool('get_agent_workview', { agent_id: 'master' });
    expect(result.isError).toBeUndefined();
    const payload = JSON.parse(result.content[0].text);
  ...
  it('supports resources/list and resources/read over JSON-RPC', async () => {
    ...
    const uris = listResult.resources.map((resource) => resource.uri);
    expect(uris).toContain('orchestrator://state/backlog');
    expect(uris).toContain('orchestrator://state/agents');

    const readResult = await sendRequest('resources/read', { uri: 'orchestrator://state/backlog' });
    expect(readResult.contents[0].mimeType).toBe('application/json');
    const backlog = JSON.parse(readResult.contents[0].text);
