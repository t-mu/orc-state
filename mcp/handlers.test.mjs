...
function seedEventsLines(lines) {
...
describe('mcp read handlers', () => {
  ...
  it('handleListAgents omits dead by default, includes dead when requested, and supports role filter', () => {
    const all = handleListAgents(dir);
    expect(all).toHaveLength(2);
    ...
    const withDead = handleListAgents(dir, { include_dead: true });
    ...
    const workers = handleListAgents(dir, { role: 'worker' });
  ...
  it('handleListStalledRuns returns stale runs with stale_for_ms', () => {
    ...
    const stale = handleListStalledRuns(dir, { stale_after_ms: 60_000, now_ms: now });
  ...
  it('handleGetTask returns task or not_found error', () => {
    const task = handleGetTask(dir, { task_ref: 'project/todo-one' });
  ...
  it('handleGetStatus returns aggregate keys and expected value shapes', () => {
    ...
    expect(status.agents.every((agent) =>
      ['agent_id', 'role', 'status', 'provider', 'active_task_ref'].every((key) => Object.hasOwn(agent, key)))).toBe(true);
    expect(status.agents.some((agent) => agent.agent_id === 'orc-dead')).toBe(false);
    ...
    expect(status.active_tasks.every((task) =>
      ['ref', 'title', 'status', 'epic_ref', 'owner'].every((key) => Object.hasOwn(task, key)))).toBe(true);
    expect(status.active_tasks.some((task) => task.status === 'done' || task.status === 'released')).toBe(false);
  ...
  it('handleGetStatus includes done and released counts when include_done_count=true', () => {
    ...
    expect(status.task_counts.done).toBe(1);
    expect(status.task_counts.released).toBe(1);
  });
  ...
  it('handleGetAgentWorkview returns idle view when the agent has no assigned work', () => {
    const workview = handleGetAgentWorkview(dir, { agent_id: 'master' });
    expect(workview.agent.agent_id).toBe('master');
  ...
  it('handleGetAgentWorkview includes blockers for owned tasks that are not actionable', () => {
    ...
    const workview = handleGetAgentWorkview(dir, { agent_id: 'orc-1' });
    const ownedBlocked = workview.queued_tasks.find((task) => task.ref === 'project/owned-blocked');
