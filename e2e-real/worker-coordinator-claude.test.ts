 ...
 *
 * Real-provider smoke test: coordinator + Claude worker blessed path.
 *
 * Opt-in: set RUN_REAL_PROVIDERS=1 to enable.
 * Skips automatically when Claude provider is unavailable (binary not found,
...
describe.skipIf(!ENABLED)('coordinator + real Claude worker smoke', () => {
  ...
  beforeAll(async () => {
    const readiness = await requireProviderReady('claude');
    if (!readiness.ok) {
      console.warn(`[claude-smoke] skipping: ${readiness.message}`);
      providerSkipped = true;
      return;
    }

    repo = createRuntimeRepo();
    const runtimeEnv = buildRuntimeEnv(repo, 'claude');
  ...
  it(
    'dispatches and completes two sequential blessed-path tasks with a real Claude worker',
    async ({ skip }) => {
      if (providerSkipped) {
        skip();
        return;
