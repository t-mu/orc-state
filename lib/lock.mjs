...
const HELD_LOCK_TOKENS = new Map();
...
export function acquireLock(lockPath) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    let storedPid;
    try {
      storedPid = JSON.parse(readFileSync(lockPath, 'utf8'))?.pid;
    } catch (e) {
      if (e?.code === 'ENOENT') continue;
...
export function withLock(lockPath, fn) {
  ...
  try {
    ...
    if (fnError) {
      throw new Error(`Lock release failed after callback error: ${releaseError.message}`, { cause: fnError });
    }
    throw releaseError;
  }
 ...
 */
export async function withLockAsync(lockPath, fn) {
  acquireLock(lockPath);
  let value;
  let fnError = null;
  try {
