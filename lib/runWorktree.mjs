...
import { RUN_WORKTREES_FILE, WORKTREES_DIR } from './paths.mjs';
import { resolveRepoRoot } from './repoRoot.mjs';

function readRunWorktrees(stateDir) {
  try {
    return JSON.parse(readFileSync(RUN_WORKTREES_FILE, 'utf8'));
  } catch {
...
export function pruneMissingRunWorktrees(stateDir, activeRunIds = []) {
  return withLock(lockPath(stateDir), () => {
...
export function ensureRunWorktree(stateDir, { runId, taskRef, agentId }) {
  if (!runId) throw new Error('runId is required');
