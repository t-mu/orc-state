...
import { listCoordinatorAgents } from './agentRegistry.mjs';
import { readEvents } from './eventLog.mjs';
import { latestRunActivityDetailMap } from './runActivity.mjs';
import { loadWorkerPoolConfig } from './providers.mjs';
...
function isManagedSlot(agentId, maxWorkers) {
...
function buildTaskCounts(backlogFile) {
  const taskStatuses = {};
...
function listDispatchReadyTasks(backlogFile) {
...
function classifyWorkerSlot(slot, activeClaim) {
...
function collectRecentFailures(events) {
  const startup = [];
  const lifecycle = [];
...
function readRunWorktrees(stateDir) {
  try {
    return readJson(stateDir, 'run-worktrees.json').runs ?? [];
  } catch {
    return [];
  }
}
 ...
 */
export function buildStatus(stateDir) {
  ...
  const runActivity = latestRunActivityDetailMap(allEvents);
...
export function buildAgentStatus(stateDir, agentId) {
  ...
  const backlogFile = readJson(stateDir, 'backlog.json');
  const agent = status.agents.list.find((entry) => entry.agent_id === agentId) ?? null;
  const activeClaims = status.claims.active.filter((claim) => claim.agent_id === agentId);
...
export function formatStatus(status) {
  ...
  if (!status.master) {
    ...
    lines.push(
      `  ${status.master.agent_id} ${status.master.provider} ${status.master.status} ${status.master.session_handle ? 'attached' : 'detached'}`,
    );
  }

  lines.push('');
  lines.push('Worker Capacity:');
  lines.push(`  configured_slots:    ${status.worker_capacity.configured_slots}`);
  lines.push(`  used_slots:          ${status.worker_capacity.used_slots}`);
  lines.push(`  available_slots:     ${status.worker_capacity.available_slots}`);
  lines.push(`  warming_slots:       ${status.worker_capacity.warming_slots}`);
  lines.push(`  unavailable_slots:   ${status.worker_capacity.unavailable_slots}`);
  lines.push(`  slot_provider:       ${status.worker_capacity.provider}`);
  lines.push(`  dispatch_ready:      ${status.worker_capacity.dispatch_ready_count}`);
  lines.push(`  waiting_for_capacity:${` ${status.worker_capacity.waiting_for_capacity}`}`);
  if (status.worker_capacity.dispatch_ready_tasks.length === 0) {
    lines.push('  queue:               (none)');
  } else {
    for (const task of status.worker_capacity.dispatch_ready_tasks.slice(0, 3)) {
      lines.push(`  queue:               ${task.ref}`);
    }
    if (status.worker_capacity.dispatch_ready_tasks.length > 3) {
      lines.push(`  queue:               +${status.worker_capacity.dispatch_ready_tasks.length - 3} more`);
    }
  }
  const spawnedSlots = status.worker_capacity.slots.filter((slot) => slot.session_handle !== null);
  ...
  lines.push('');
  lines.push(`Active Runs (${status.claims.total}):`);
  lines.push(`  awaiting_run_started: ${status.claims.awaiting_run_started ?? 0}`);
  lines.push(`  in_progress:          ${status.claims.in_progress ?? 0}`);
  lines.push(`  stalled:              ${status.claims.stalled ?? 0}`);
  if (status.claims.active.length === 0) {
    lines.push('  (none)');
  } else {
    for (const c of status.claims.active) {
  ...
  lines.push('');
  lines.push(`Finalization (${status.finalization.total}):`);
  lines.push(`  awaiting_finalize:        ${status.finalization.awaiting_finalize}`);
  lines.push(`  finalize_rebase_requested:${` ${status.finalization.finalize_rebase_requested}`}`);
  lines.push(`  finalize_rebase_in_progress:${` ${status.finalization.finalize_rebase_in_progress}`}`);
  lines.push(`  ready_to_merge:           ${status.finalization.ready_to_merge}`);
  lines.push(`  blocked_preserved:        ${status.finalization.blocked_finalize}`);
  if (status.finalization.active.length === 0) {
    lines.push('  (none)');
  } else {
    for (const claim of status.finalization.active) {
  ...
  lines.push('');
  lines.push(`Recent Failures (${status.failures.startup.length + status.failures.lifecycle.length}):`);
  if (status.failures.startup.length === 0 && status.failures.lifecycle.length === 0) {
    lines.push('  (none)');
  } else {
    for (const failure of status.failures.startup) {
      lines.push(`  startup ${failure.agent_id ?? 'n/a'} ${failure.run_id ?? 'n/a'} ${truncate(failure.reason)}`);
    }
    for (const failure of status.failures.lifecycle) {
      lines.push(`  ${failure.event} ${failure.agent_id ?? 'n/a'} ${failure.run_id ?? 'n/a'} ${truncate(failure.reason)}`);
    }
  }

  lines.push('');
  lines.push(`Tasks (${status.tasks.total} total):`);
  for (const [s, count] of Object.entries(status.tasks.counts).sort()) {
    lines.push(`  ${s.padEnd(14)} ${count}`);
  }

  if (status.eventReadError) {
    lines.push('');
    lines.push(`Event log warning: ${status.eventReadError}`);
  }

  return lines.join('\n');
}

export function formatAgentStatus(agentStatus, agentId) {
  const lines = [];
  ...
  lines.push('');
  lines.push(`Role: ${agentStatus.agent.role ?? 'worker'}`);
  lines.push(`Status: ${agentStatus.agent.status ?? 'unknown'}`);
  lines.push(`Provider: ${agentStatus.agent.provider ?? 'unknown'}`);

  lines.push('');
  lines.push(`Assigned Tasks (${agentStatus.assigned_tasks.length}):`);
  if (agentStatus.assigned_tasks.length === 0) {
    lines.push('  (none)');
  } else {
    for (const claim of agentStatus.assigned_tasks) {
  ...
  lines.push('');
  lines.push(`Queued Owned Tasks (${agentStatus.queued_tasks.length}):`);
  if (agentStatus.queued_tasks.length === 0) {
    lines.push('  (none)');
  } else {
    for (const task of agentStatus.queued_tasks) {
