...
function setTTY(stdin: boolean | undefined, stdout = stdin) {
    ...
    describe('validate function', () => {
      async function getValidate() {
        ...
        vi.mocked(input).mockImplementation((opts) => {
          ...
          return Promise.resolve('worker-01') as ReturnType<typeof input>;
        });
        await promptAgentId(null);
        return capturedOpts!.validate as ((v: string) => unknown);
      }
...
describe('promptWorkerRole()', () => {
  ...
  describe('interactive', () => {
    ...
    it('only offers worker and reviewer roles', async () => {
      ...
      expect(call?.choices.some((choice) => (choice as { value?: unknown }).value === 'master')).toBe(false);
    });
  });
});
