...
} from './claimManager.mjs';
import { nextEligibleTask, nextEligibleTaskFromBacklog } from './taskScheduler.mjs';

// ── Helpers ────────────────────────────────────────────────────────────────

function makeBacklog(tasks = []) {
  return { version: '1', epics: [{ ref: 'orch', title: 'Orch', tasks }] };
}

function makeTask(ref, status = 'todo', deps = []) {
  return { ref, title: ref, status, depends_on: deps };
}

function seed(dir, { tasks = [makeTask('orch/init')], claims = [] } = {}) {
...
function readBacklog(dir) { return JSON.parse(readFileSync(join(dir, 'backlog.json'), 'utf8')); }
function readClaims(dir)  { return JSON.parse(readFileSync(join(dir, 'claims.json'),  'utf8')); }
...
describe('nextEligibleTask', () => {
  ...
  it('skips unknown task types and selects known dispatchable task', () => {
    seed(dir, {
      tasks: [
        { ...makeTask('orch/plan'), task_type: 'planning' },
  ...
  it('does not route unknown task types to any agent', () => {
    const backlog = makeBacklog([
      { ...makeTask('orch/review'), task_type: 'review' },
      { ...makeTask('orch/impl'), task_type: 'implementation' },
    ]);
    expect(nextEligibleTaskFromBacklog(backlog, { agent_id: 'rev-1', role: 'reviewer', capabilities: [] }))
      .toBe('orch/impl');
    expect(nextEligibleTaskFromBacklog(backlog, { agent_id: 'wrk-1', role: 'worker', capabilities: [] }))
      .toBe('orch/impl');
  });
  ...
  it('requires planning_state=ready_for_dispatch when planning_state is present', () => {
    seed(dir, {
      tasks: [
        { ...makeTask('orch/a'), planning_state: 'proposal' },
  ...
  it('routes only tasks matching required_capabilities', () => {
    ...
    expect(nextEligibleTaskFromBacklog(backlog, { agent_id: 'worker-1', capabilities: ['ui'] })).toBe('orch/ui');
  });
});
