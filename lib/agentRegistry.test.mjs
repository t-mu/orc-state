...
} from './agentRegistry.mjs';

function seedDir(dir) {
...
describe('registerAgent', () => {
  ...
  it('throws when agent_id is missing', () => {
    expect(() => registerAgent(dir, { provider: 'claude' })).toThrow('agent_id');
  });

  it('throws when provider is missing', () => {
    expect(() => registerAgent(dir, { agent_id: 'agent-01' })).toThrow('provider');
  });
  ...
  it('throws when role is unsupported', () => {
    expect(() => registerAgent(dir, { agent_id: 'agent-01', provider: 'codex', role: 'manager' }))
