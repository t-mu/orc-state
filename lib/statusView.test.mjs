...
import { buildAgentStatus, buildStatus, formatAgentStatus, formatStatus } from './statusView.mjs';

let dir;
beforeEach(() => { dir = mkdtempSync(join(tmpdir(), 'orch-status-test-')); });
afterEach(() => { rmSync(dir, { recursive: true, force: true }); });

function writeState({ agents = [], claims = [], tasks = [], runWorktrees = [] } = {}) {
...
describe('buildStatus', () => {
  ...
  it('separates the master from managed worker capacity', () => {
    writeState({
      agents: [
        { agent_id: 'master', provider: 'claude', role: 'master', status: 'running' },
        { agent_id: 'orc-1', provider: 'codex', role: 'worker', status: 'idle' },
      ],
    });
  ...
  it('lists only active (non-terminal) claims', () => {
    writeState({
      agents: [{ agent_id: 'orc-1', provider: 'codex', role: 'worker', status: 'running' }],
      claims: [
        {
          run_id: 'run-1',
          task_ref: 'a/1',
  ...
  it('counts dispatch-ready work waiting for capacity', () => {
    writeState({
      agents: [{ agent_id: 'orc-1', provider: 'codex', role: 'worker', status: 'running', session_handle: 'pty:orc-1' }],
      tasks: [
...
describe('formatStatus', () => {
  ...
  it('renders master, worker capacity, and active runs sections', () => {
    writeState({
      agents: [{ agent_id: 'master', provider: 'claude', role: 'master', status: 'running' }],
      claims: [{
        run_id: 'run-1',
        task_ref: 'orch/task-1',
        agent_id: 'orc-1',
...
describe('buildAgentStatus', () => {
  it('returns agent-specific active claims and queued owned tasks', () => {
    writeState({
      agents: [{ agent_id: 'agent-01', provider: 'claude', role: 'worker', status: 'running' }],
      claims: [{
        run_id: 'run-1',
        task_ref: 'orch/task-1',
        agent_id: 'agent-01',
      ...
      tasks: [
        { ref: 'orch/task-1', title: 'Task 1', status: 'claimed', owner: 'agent-01', planning_state: 'ready_for_dispatch' },
        { ref: 'orch/task-2', title: 'Task 2', status: 'todo', owner: 'agent-01', planning_state: 'ready_for_dispatch' },
        { ref: 'orch/task-3', title: 'Task 3', status: 'todo', owner: 'agent-02', planning_state: 'ready_for_dispatch' },
      ],
    });
    writeEvents([]);

    const status = buildAgentStatus(dir, 'agent-01');
