...
function writeSpec(baseDir, name, { ref, epic, status, title = 'Example Title' } = {}) {
  const frontmatter = [
    '---',
    ...(ref ? [`ref: ${ref}`] : []),
    ...(epic ? [`epic: ${epic}`] : []),
...
describe('syncBacklogFromSpecs', () => {
  it('adds a missing task from a spec file with status todo', async () => {
    ...
    const { syncBacklogFromSpecs } = await import('./backlogSync.mjs');
  ...
  it('runs backlog sync during coordinator startup before the first tick loop', async () => {
    ...
    vi.doMock('./backlogSync.mjs', () => ({ syncBacklogFromSpecs }));

    const { main, doShutdown } = await import('../coordinator.mjs');
    await main();
